...
class NewDataClassAction : AnAction(IconLoader.getIcon("/icons/bloc_icon16.png")) {
	...
	override fun actionPerformed(event: AnActionEvent) {
		...
		val name = Messages.showInputDialog(
				"Enter a name for the data class (PascalCase)",
				"New Alza Data Class",
				null,
				null,
		...
		if (directory.findSubdirectory(bloc.name) != null) {
			Messages.showErrorDialog("Class with the same name already exists", "Alza Data Class")
			return
		}

		WriteCommandAction.runWriteCommandAction(event.project) {
			TemplateBuilder.buildDataClass(name, project, directory)
		}
	}
